...
class dropDownMenuOpenMode(
    openedIndexName: String = "",
    open: openMode = openMode.CLOSED,
    intOffset: IntOffset = IntOffset(0, 0)
) {
    // Используем делегаты 'by mutableStateOf'
    var openedIndexName by mutableStateOf(openedIndexName)
    var open by mutableStateOf(open)
    var intOffset by mutableStateOf(intOffset)

    fun openDropDown(openIndexName: String, openModeForSource: openMode, offset: IntOffset = IntOffset(0, 0)) {
        openedIndexName = openIndexName
        open = openModeForSource
        this.intOffset = offset
    }

    fun closeDropDown() {
        openedIndexName = ""
        open = openMode.CLOSED
    }
}

@Composable
fun createDropDownPlaylist(
